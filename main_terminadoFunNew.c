...
#define TOPE_DE_BYTES 8192  /* es a longitud neta, sin los apóstrofos ni el cero final */
...
typedef enum {NULO, FIN_DE_ARCHIVO, BEGIN, END, ABREPAREN, CIERRAPAREN, READLN, WRITE, WRITELN, VAR, CADENA, IDENT, NUMERO, PUNTO, COMA, PTOCOMA, MAS,
               CONST,IGUAL,MAYOR, MENOR, PROCEDURE, ASIGNAR, CALL, IF,ELSE,HALT, THEN, WHILE,FOR, DO, ODD, DISTINTO, MENORIGUAL, MAYORIGUAL,MENOS, MULTIPLICAR, DIVIDIR } tTerminal;
...
void codigoTerminacion (byte memoria[], int *tope, int *contadorVariables);
...
void cargarByteEn(byte dato, byte memoria[],int pos);
...
int main(int argc, char* argv[]) {
    ...
    if (archivo == NULL) {
        ...
        generarNombreEjecutable(argv[1], nombreExe);

        grabarArchivoEjecutable(memoria, &tope,nombreExe);
...
tSimbolo programa (tSimbolo s, FILE *archivo, byte memoria[], int *tope){
    ...
    if(s.simbolo==PUNTO){
        s=aLex(archivo);
        codigoTerminacion(memoria,tope,&contadorVariables);
...
void codigoTerminacion (byte memoria[], int *tope, int *contadorVariables){
        ...
        int imageOfBase=leerIntDe(212, memoria);
        int baseOfCode=leerIntDe(200, memoria);

        int imageMasBaseMasTopeMenos=leerIntDe(212, memoria)+leerIntDe(200, memoria)+(*tope-0x200);
    ...
    int dale=(*tope - 0x200);
    int bf=leerIntDe(188,memoria);
    cargarIntEn((*tope - 0x200),memoria, 188);   // Pag. 31, parrafo 7
    cargarIntEn((*tope - 0x200),memoria, 424);   // Ajusta campos SizeOfCodeSection y SizeOfRawData con el tamaño final de .text


    int sizeOfCodeSection = leerIntDe(188,memoria);                // Pag. 31, parrafo 8
    int sizeOfRawData = leerIntDe(424,memoria);                    // Ajusta los campos SizeOfImage (240) y BaseOfData (208) según los campos
    int sectionAlignment = leerIntDe(216,memoria);
           // SizeOfCodeSection (188), SizeOfRawData (424) y SectionAlignment (216).
    int estocuandoda=(2 + sizeOfCodeSection / sectionAlignment) * sectionAlignment;
    cargarIntEn((2 + sizeOfCodeSection / sectionAlignment) * sectionAlignment,memoria,240);
    cargarIntEn((2 + sizeOfCodeSection / sectionAlignment) * sectionAlignment,memoria,208);
...
tSimbolo bloque (tSimbolo s, FILE *archivo, tablaDeIdent tabla, int base, byte memoria[], int *tope, int *contadorVariables){
    ...
    if(distancia > 0){                              // Si el tamaño del bloque es mayor que cero, hago el fixup del salto
        cargarIntEn(distancia, memoria, (inicio-4)); // Si el tamaño del bloque es mayor que cero, hago el fixup del salto
...
tSimbolo proposicion (tSimbolo s, FILE *archivo, tablaDeIdent tabla,int base, int posUltimoIdent,byte memoria[],int *tope, int * contadorVariables){
    int p, posPrevCond, posPostCond, origSalto, destSalto, origSaltoElse, destSaltoElse, dist, distancia;
    ...
    case IF:
        ...
        s = proposicion(s,archivo,tabla,base,posUltimoIdent,memoria, tope, contadorVariables);
        destSalto=(*tope);
        cargarIntEn(destSalto-origSalto,memoria,origSalto-4);//cargamos la distancia en la posiciòn que sigue a e9 dentro de condición

        if (s.simbolo == ELSE){
            cargarIntEn(destSalto-origSalto+5,memoria,origSalto-4);
            ...
            cargarInt(0,memoria,tope);
             ...
             s = proposicion(s,archivo,tabla,base,posUltimoIdent,memoria, tope,contadorVariables);
             destSaltoElse=(*tope);
             cargarIntEn(destSaltoElse-origSaltoElse,memoria,origSaltoElse-4);
        }
        break;
    case WHILE:
        s=aLex(archivo);
        ...
        else error(9,s.cadena);                              // Se esperaba DO
        s = proposicion(s,archivo,tabla,base,posUltimoIdent,memoria, tope,contadorVariables);
        destSalto=(*tope);

        dist=posPrevCond - (*tope+5);                       // Cantidad de bytes a saltar hacia atras
        cargarByte(0xE9,memoria,tope);                      // JMP dir, salto incondicional hacia atras
        cargarInt(dist,memoria,tope);                       // ... (vuelve hasta el inicio de la condicion)
        cargarIntEn((*tope)-posPostCond,memoria,posPostCond-4); // Fixup del salto E9 de la condicion del while
    ...
    case READLN:
        ...
        if(s.simbolo==IDENT){
            ...
            if(p != -1){
                ...
                int distancia = 0x590-(*tope+4);// call
                cargarInt(distancia,memoria,tope);
    ...
    case WRITE:
        ...
        if(s.simbolo==CADENA){
            int posCadena;
            posCadena=(leerIntDe(212, memoria)+leerIntDe(204, memoria)+(*tope-0x200)+15);
            cargarByte(0xB8,memoria,tope);
            cargarInt(posCadena,memoria,tope); //MOV EAX // Ubicacion absoluta de la cadena
            //int distancia = (0x3E0 - (*tope +5));// Rutina de impresion por pantalla
            cargarByte(0xE8, memoria, tope); // Rutina de impresion por pantalla
            ...
            s=aLex(archivo);
              ...
              cargarPopEax(memoria,tope);
              cargarByte(0xE8,memoria,tope);
              cargarInt(0x420 - (*tope + 4),memoria,tope);
        ...
        if(s.simbolo==ABREPAREN){
            ...
            if(s.simbolo==CADENA){
               int posCadena;
                ...
                cargarInt(0x3E0 - (*tope +4),memoria,tope);// call
                cargarByte(0xE9,memoria,tope); //JMP
                cargarInt(strlen(s.cadena)-1,memoria,tope); //JMP
                int i;
                for(i=1;i<strlen(s.cadena)-1;i++){
            ...
            while(s.simbolo==COMA){
                ...
                if(s.simbolo==CADENA){
                    ...
                    cargarByte(0xB8,memoria,tope);
                    cargarInt(posCadena,memoria,tope); //MOV EAX // Ubicacion absoluta de la cadena
                                                        // Rutina de impresion por pantalla
                    cargarByte(0xE8, memoria, tope); // call
                    cargarInt(0x3E0 - (*tope +4),memoria,tope);// call
                    cargarByte(0xE9,memoria,tope); //JMP
                    cargarInt(strlen(s.cadena)-1,memoria,tope); //JMP
                    ...
                    s = expresion(s,archivo,tabla,base,posUltimoIdent, memoria, tope);
                    cargarPopEax(memoria,tope); // pop Eax
                    //int distancia = 0x420-(*tope+4); //call
                    cargarByte(0xE8, memoria, tope);
                    cargarInt(0x420-(*tope+4),memoria,tope);
                }
            ...
            cargarByte(0xE8, memoria, tope);
            cargarInt(0x410-(*tope+4),memoria,tope);
...
tSimbolo condicion (tSimbolo s, FILE *archivo, tablaDeIdent tabla,int base, int posUltimoIdent,byte memoria[], int *tope){
    ...
    if(s.simbolo==ODD){
        ...
        cargarByte(0x01,memoria,tope);                                      // ... (ab)
        ...
        cargarByte(0x39,memoria, tope);                                      // CMP EBX, EAX     => Compara EBX y EAX segun los operadores del switch y salta
        cargarByte(0xC3,memoria, tope);                                      // ...              la cantidad de bytes indicadas despues del Jump If

        switch(operador){
            case IGUAL:
            cargarByte(0x74,memoria, tope);                                    // JE dir     => Jump if equal
            cargarByte(0x05,memoria, tope);                                    // Salta 5 bytes (una instruccion)
            break;
            case DISTINTO:
            cargarByte(0x75,memoria, tope);                                    // JNE dir    => Jump if not equal
            cargarByte(0x05,memoria, tope);
            break;
            case MAYOR:
            cargarByte(0x7F,memoria, tope);                                    // JG dir    => Jump if greater than
            cargarByte(0x05,memoria, tope);
            break;
            case MENOR:
            cargarByte(0x7C,memoria, tope);                                    // JL dir    => Jump if less than
            cargarByte(0x05,memoria, tope);
            break;
            case MAYORIGUAL:
            cargarByte(0x7D,memoria, tope);                                    // JGE dir    => Jump if greater than or equal
            cargarByte(0x05,memoria, tope);
            break;
            case MENORIGUAL:
            cargarByte(0x7E,memoria, tope);                                    // JLE dir    => Jump if less than or equal
            cargarByte(0x05,memoria, tope);
            break;
...
tSimbolo termino (tSimbolo s, FILE *archivo, tablaDeIdent tabla,int base, int posUltimoIdent,byte memoria[], int *tope){
    ...
    while(s.simbolo == MULTIPLICAR || s.simbolo == DIVIDIR){
        ...
        if(op==MULTIPLICAR){
            ...
            cargarByte(0xEB,memoria,tope);
            cargarByte(0x50,memoria,tope);//PUSH EAX
        }else { // acà entramos a dividir
            cargarPopEax(memoria,tope); // pop Eax
            cargarByte(0x5B,memoria,tope); //pop EBX
            cargarByte(0x93,memoria,tope);
            cargarByte(0x99,memoria,tope);
            cargarByte(0xF7,memoria,tope);//IDIV
            cargarByte(0xFB,memoria,tope);
            cargarByte(0x50,memoria,tope); // PUSH EAX
        }
    }
    ...
    case IDENT:
        ...
        if (p == -1) error(15, s.cadena);                                    // identificador no encontrado
          if (tabla[p].tipo == PROCEDURE) {
            ...
            cargarByte(0x8B,memoria,tope);                                  // MOV EAX[EDI + ....] (8B 87 gh ef cd ab)
            cargarByte(0x87,memoria,tope);                                  // ....
            cargarInt(tabla[p].valor,memoria,tope);                         // Direccionamiento indexado, el valor de VAR es el offset de bytes respecto de EDI
            cargarByte(0x50,memoria,tope);                                  // PUSH EAX
          } else if (tabla[p].tipo == CONST) {                          // CONST
            cargarByte(0xB8,memoria,tope);                                  // MOV EAX .... (B8 gh ef cd ab)
            cargarInt(tabla[p].valor,memoria,tope);                         // Direccionamiento inmediato
            cargarByte(0x50,memoria,tope);                                  // PUSH EAX
          }
        s=aLex(archivo);
        break;
...
void cargarIntEn (int n, byte memoria [], int dirMem){
    ...
    if (n<0){
        ...
        // Como la PC es little endian, se insertan al reves, de a pares
  cargarByteEn(num % 0x100,memoria, dirMem); // Ultimos 2 digitos
  cargarByteEn(num / 0x100 % 0x100,memoria,  dirMem + 1); // los 2 siguientes
  cargarByteEn(num / 0x10000 % 0x100,memoria,  dirMem + 2); // los 2 siguientes
  cargarByteEn(num / 0x1000000 % 0x100,memoria,  dirMem + 3); // los 2 siguientes
}
